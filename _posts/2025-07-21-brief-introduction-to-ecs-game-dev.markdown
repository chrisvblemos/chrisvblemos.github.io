...
Recently I decided to dip my toes into the ECS architecture for game development inside Unity. My first experience with ECS was during the development of a custom game engine written in C++. Now, I wanted to see how this architecture could look like in bigger projects like Unity - this is where I found out about Unity DOTS, which I'm playing with at the moment. 
...
ECS stants for Entity-Component-System. It's just a name that will make sense later in this text. It's a different way to structure your data and logic so that you can make your software run faster, far different than Object Oriented Programming. 
...
As a bonus of all these decisions, ECS works betters with multithreaded jobs, since it is easier to delegate the data operations to a later stage and avoid race conditions when using pointers directly.
...
In the context of OOP game development, we typically have a class for GameObjects and Component classes that introduce behaviour to our game objects. We then add the logic inside this behaviours and iterate through them calling an Update method each frame of our game.
...
To handle this, people came up with Archetypes. Archetypes are just a combination of components that an entity might have. In practice, you build arrays that contains combinations of components, storing entities that have this combination of components.
